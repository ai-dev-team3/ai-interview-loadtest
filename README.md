...
**한 건의 GPU 시간은 1.4초다.** 그런데 HTTP 경로에서 STT는 1.5~2.2초(1명), 3.9초(5명)로
찍혔다. 차이는 백그라운드 분석과의 경합인데, **CPU 경합이 아니라 GPU 경합이다.**

`scorer.py`의 유사도 모델 두 개 중 `paraphrase-multilingual-mpnet-base-v2`는
`SentenceTransformer`로 로드되고, 이건 **같은 GPU(cuda:0)에 올라간다.** 서버 기동 로그가
그대로 말해준다:

```
[sentence_transformers.base.model] No device provided, using cuda:0
```

즉 답변마다 백그라운드로 도는 임베딩 유사도 계산이 **SenseVoice STT와 같은 GPU 한 장을 두고
다툰다.** (나머지 모델 `snunlp/KR-SBERT`는 `.to(device)`가 없어 CPU에 남고, 피치 분석은
librosa라 CPU다. CPU 경합도 일부 있지만 STT를 느리게 만드는 쪽은 GPU다.)

이것이 **LLM 지연을 0초로 만들어도 `/answer`(동시 1명)가 3.37초 → 3.45초로 줄지 않은** 이유다.
LLM 대기가 사라지면 백그라운드 분석이 그만큼 일찍 시작되고, 그 임베딩이 GPU에 끼어들어 다음
STT를 밀어낸다. **LLM을 아무리 빠르게 만들어도 `/answer`는 빨라지지 않는다** — 비워진 자리를
같은 GPU를 쓰는 백그라운드 작업이 바로 채운다.

이 관점에서 보면 GPU를 쓰는 작업은 셋이다: **STT(사용자를 기다리게 함) / 임베딩(백그라운드) /
그리고 둘 다 GPU 한 장을 공유한다.** 사용자를 기다리게 하는 STT에 우선순위를 주려면, 백그라운드
임베딩을 CPU로 내리거나 별도 장비/프로세스로 빼는 것부터 검토해야 한다 — 이건 스트리밍 STT보다
훨씬 싼 변경이다.
...
0. **백그라운드 임베딩을 GPU에서 내린다 (가장 싼 변경).** 지금은 사용자를 기다리게 하는 STT와
   백그라운드 유사도 임베딩이 같은 GPU를 나눠 쓴다. `SentenceTransformer(..., device="cpu")`
   한 줄이면 급한 경로(STT)가 GPU를 독점한다. 백그라운드는 몇 초 늦어도 사용자가 모른다
   (분석 수렴 기준은 60초인데 실측 15.8초로 여유가 크다).
