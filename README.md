...
HTTP를 통해 재면 백그라운드 분석(임베딩 유사도 약 3 CPU-초, 피치)이 같은 프로세스에서 같이
돌아 STT 시간에 CPU 경합이 섞인다. 그래서 `SenseVoiceClient.transcribe`만 직접 동시 N개로
때렸다. 75초짜리 답변 오디오 기준:
...
찍혔다. 차이는 같은 프로세스에서 도는 백그라운드 분석의 CPU 경합이다. 실제로 LLM 지연을
0초로 만들어도 `/answer`(동시 1명)는 3.37초 → 3.45초로 **줄지 않았다** — 사라진 LLM 대기
시간만큼 백그라운드 CPU 작업이 앞당겨져 STT와 부딪혔기 때문이다.
