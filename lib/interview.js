...
import {
    AUDIO, ANSWERS, BASE, JPEG, MAX_ANSWERS, PREPARE_SECONDS, START_JITTER, VIDEO_MODE, WS_BASE,
    analysisConvergence, analysisTimeouts, headers, pickAnswer, recordFailure, wsFrameRtt,
} from './config.js';
import { FRAME_INTERVAL_MS, frame, jpegFrame } from './landmarks.js';

// 페이로드는 실행 내내 고정이므로 한 번만 만든다. jpeg 모드면 서버가 접속마다 MediaPipe 로 처리.
const JPEG_PAYLOAD = VIDEO_MODE === 'jpeg' ? jpegFrame(JPEG) : null;
function videoFrame() {
    return JPEG_PAYLOAD !== null ? JPEG_PAYLOAD : frame();
}
...
export function submitAnswer(u, sessionId, questionOrder, audio = AUDIO) {
    const res = http.post(
...
export function runInterview(u) {
    // 도착 시각 분산 (평균 시나리오). lockstep 답변 제출을 막는다.
    if (START_JITTER > 0) sleep(Math.random() * START_JITTER);
    ...
    for (;;) {
        ...
        // 답변마다 길이가 다르다 (30~90초 삼각분포) — STT 처리 시간도 그만큼 달라진다.
        const ans = pickAnswer();
        streamExpression(u, sessionId, order, ans.seconds); // 답변하는 동안 영상 소켓이 열려 있다

        const answered = submitAnswer(u, sessionId, order, ans.audio);
        if (!answered) return;
    ...
    const closing = http.post(
        ...
        { audio: http.file(ANSWERS[0].audio, 'answer.webm', 'audio/webm') },  // 마지막 한마디는 짧게(30초)
