...
export const ANSWERS = [
    { seconds: 30, audio: open('../fixtures/answer_30s.webm', 'b') },
    { seconds: 60, audio: open('../fixtures/answer_60s.webm', 'b') },
    { seconds: 90, audio: open('../fixtures/answer_90s.webm', 'b') },
];

// 최악 시나리오용: 답변 길이를 고정(초). 0 이면 아래 분포를 따른다.
const FORCE_ANSWER = Number(__ENV.FORCE_ANSWER || 0);

/** 답변 길이 분포: 짧게 30초 30%, 중간 60초 40%, 길게 90초 30%. FORCE_ANSWER 면 고정. */
export function pickAnswer() {
    if (FORCE_ANSWER) {
        return ANSWERS.find((a) => a.seconds === FORCE_ANSWER) || ANSWERS[ANSWERS.length - 1];
    }
    const r = Math.random();
    if (r < 0.3) return ANSWERS[0];
    if (r < 0.7) return ANSWERS[1];
    return ANSWERS[2];
}
